...
import { BLINK_EAR_THRESHOLD, BLINK_HISTORY_MS } from '../constants/thresholds'
...
export class BlinkDetector {
  ...
  update(
    ...
    now = Date.now()
  ): { ear: number; blinkCount: number; blinksPerMinute: number } {
    ...
    const blinksPerMinute = this.blinkTimestamps.length

    return { ear, blinkCount: this.blinkCount, blinksPerMinute }
  }
}
